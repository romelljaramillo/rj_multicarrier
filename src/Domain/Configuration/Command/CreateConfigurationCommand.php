<?php
/**
 * DTO for creating Configuration data.
 */
...
final class CreateConfigurationCommand
{
    ...
    /**
     * Builds the command from context defaults.
     *
     * @param int[] $contextShopIds
     */
    public static function fromContext(ConfigurationView $contextView, array $contextShopIds): self
    {
        $command = new self();
        $data = $contextView->getFormData();
        ...
        $command->ensureShopAssociation($contextView->getShopId(), $contextShopIds);

        return $command;
    }

    /**
     * Factory from raw form payload (legacy/CLI contexts).
     *
     * @param array<string, mixed> $data
     * @param int[] $contextShopIds
     */
    public static function fromArray(array $data, int $fallbackShopId, array $contextShopIds): self
    {
        $command = new self();
        $command->firstname = (string) ($data['firstname'] ?? '');
        $command->lastname = (string) ($data['lastname'] ?? '');
