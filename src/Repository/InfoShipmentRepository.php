...
class InfoShipmentRepository extends ServiceEntityRepository
{
    ...
    {
        parent::__construct($registry, InfoShipment::class);
    }
    ...
    public function findForExport(array $filters): array
    {
        ...
        $qb = $this->createQueryBuilder('package')
            ->select([
                ...
                'package.id AS id_info_shipment',
        ...
        if (!empty($filters['id_info_shipment'])) {
            $qb->andWhere('package.id = :id')
                ->setParameter('id', (int) $filters['id_info_shipment']);
        }
    ...
    public function getPackageByOrder(int $orderId, int $shopId): ?array
    {
        ...
        if (!$package instanceof InfoShipment) {
            return null;
        }
        ...
        return [
            'id_info_shipment' => $package->getId(),
    ...
    private function applyShopRestriction(\Doctrine\ORM\QueryBuilder $qb): void
    {
        ...
        // Use ORM join against the InfoShipment->shops collection (InfoShipmentShop entity)
