...
final class ConfigurationType extends AbstractType
{
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        $builder
            ->setMethod('POST')
            ->add('id_configuration', HiddenType::class, [
                'required' => false,
            ])
            ...
            ->add('RJ_ETIQUETA_TRANSP_PREFIX', TextType::class, [
                'label' => 'Prefijo etiqueta transporte',
                'required' => false,
                'constraints' => [new Assert\Length(['max' => 64])],
            ])
            ->add('RJ_MODULE_CONTRAREEMBOLSO', ChoiceType::class, [
                'label' => 'Módulo contrareembolso',
                'required' => false,
                'placeholder' => 'Selecciona un módulo',
                'choices' => $options['cod_module_choices'],
                'choice_translation_domain' => false,
            ]);
    ...
    public function configureOptions(OptionsResolver $resolver): void
    {
        ...
        $resolver->setAllowedTypes('cod_module_choices', 'array');
    }
}
